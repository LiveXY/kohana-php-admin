...
(function ($) {
	...
	<div class="userControl-wrapper hide">\
		<div class="userinfo-box">\
			<div class="userinfo-avatar"><img src="" width="70" height="70" class="avatar"><br /><span class="uid"></span><br /><span class="umotor hide">机器人</span></div>\
			<div class="userinfo">\
		...
		me.bind = function(obj) {
			me.find(options.className).live('mousemove', function(){
				...
				menu.hideMenu();
				menu.showMenu($(this), 'left', 'top', -5, -5);
				menu.attr('uid', uid);
			});
		...
		me.init = function() {
			...
			menu.mouseleave(function(){ menu.hideMenu(); });
	...
	$.fn.smallLang = function (options) {
		...
		var defaults = { lang: { 'zh_TW': '繁体', 'en_US': '英文' }, def: "zh_TW", change: null };
		options = $.extend(defaults, options);
	...
	$.fn.LangBox = function (options) {
		...
		var me = this, defaults = { lang: { 'zh_TW': '繁体', 'en_US': '英文' }, def: 'zh_TW', width: 405, data: null, textarea: false };
		options = $.extend(defaults, options);
	...
	$.fn.navTree = function (itemClick, expandAll) {
		...
		///	 簡單的樹
		/// </summary>
		/// <param name="itemClick" type="function">樹的页子單擊事件</param>
		...
		var templates = '\
			<div class="checkboxlist checkboxlist{0} {2}">\
				...
				<div class="checkboxlist-opterate">\
					<div class="checkboxlist-search"><input type="text" class="txtSearch" default="请输入搜索关键字" /></div>\
					<div class="checkboxlist-left">\
						<label><input type="checkbox" class="chkAll" /> 选择全部</label>&nbsp;&nbsp;\
						<input type="button" class="btnReset" title="恢复默认值" />\
					</div>\
				</div>\
	...
	$.fn.checkComboBoxList = function (options) {
		...
		var $this = this, templates = '\
			...
			</div></div>', templates_item = '<li>{1}<input type="button" class="btnDelete" title="删除" val="{0}" /></li>';
		var defaults = {
	...
	$.fn.checkComboBoxTree = function (options) {
		...
		this.init = function (me) {
			...
			meprev2.text(options.title == '' ? "-请选择区域-" : options.title).click(function () {
	...
	$.fn.spager = function (options) {
		var me = this, defaults = { page: 1, pageSize: 10, total: 0, prevText: "&nbsp;", nextText: "下一页&nbsp;", linkText: "{0}", className: "", call: null, url: "", showOnePage: true };
		options = $.extend(defaults, options);
	...
	$.fn.pager = function (options) {
		...
		///	 &#10;	showGo				   是否顯示輸入页碼條轉
		///	 &#10;	showOnePage			  是否顯示只有一页的页碼
		/// </param>
		var me = this, defaults = { page: 1, pageSize: 10, total: 0, prevText: "上一页", nextText: "下一页", linkText: "{0}", totalText: "{0}条记录", className: "pager", call: null, url: null, align: "right", showGo: false, showOnePage: true };
		options = $.extend(defaults, options);
		var a_templates = '<a href="{0}" value="{2}">{1}</a>&nbsp;', span_templates = '<span>{0}</span>&nbsp;', em_templates = '<strong{1}>{0}</strong>&nbsp;',
			input_templage = '<input type="text" maxlength="9" size="2" value="{0}" onfocus="this.value=this.defaultValue;this.select();" title="可以输入页码按回车键自动跳转" class="txt" name="gopage">',
			pages = parseInt(options.total / options.pageSize + (options.total % options.pageSize == 0 ? 0 : 1)),
	...
	$.fn.fixTH = function (rows) {
		...
		$(".body").scroll(function () {
			var top = me.doc.scrollTop(), i = 0;
			if (top > me.top) {
				if (!me.fix) { me.fix = true; me.cloneTable.css({ "position": "absolute", "top": "0px", "left": me.L, "width": me.W }).show(); i++; }
			} else {
		...
		$(window).resize(function () {
			...
			me.top = $this.offset().top;
			me.L = $this.offset().left;
			me.fix = false;
		}).focus(function () {
			$(".body").scroll();
		}).resize();
		...
		var template_form = '<iframe name="jupload{1}" class="jupload_iframe" id="jupload_iframe{1}" />\
			...
			template_box = '<div class="jupload-box" id="jupload-box{1}">\
								...
								<div class="jupload-opterate">\
									<a href="#" class="btn"><img src="/client/images/ico/up.gif"> {2}</a>\
	...
	$.fn.uploadSwf = function (options, callback) {
		...
		this.event = function (me, time) {
			$("#jupload-file"+time).uploadify({
				...
				swf					: '/client/js/uploadify.swf',
				uploader 			: options.url,
				cancelImage 		: '/client/images/uploadify-cancel.png',
	...
	$.fn.uploadImg = function (options, callback, click) {
	    var $this = this; defaults = { url: "", title: "上傳圖片文件" };
	    options = $.extend(defaults, options);
	    options.width = options.width || $this.width();
	    options.height = options.height || $this.height();

	    var template_form = '<iframe name="uploadImg{1}" class="uploadImg_iframe" id="uploadImg_iframe{1}" /><form action="{0}" target="uploadImg{1}" id="uploadImg_form{1}" method="post" enctype="multipart/form-data" class="uploadImg_form" name="uploadImg_form"></form>',
	        template_box = '<div class="uploadImg-box" id="uploadImg-box{0}"><div class="uploadImg-btn">{1}</div><input type="file" class="uploadImg-file" size="13" name="Filedata" tabindex="-1" /></div>';

	    this.event = function (me, rand) {
	    	var meprev = me.prev(), iframe = $("#uploadImg_iframe"+rand),
	    		btn = meprev.find(".uploadImg-btn"),
	    		file = meprev.find(".uploadImg-file");

	    	btn.click(function(){ return false; });
	    	iframe.load(function(){
	    		var data = $(iframe[0].contentWindow.document.body).text();
	    		if (data == '') return false;
	    		if (callback) callback(data, data, data);
	    		$(file).replaceWith(file.clone(true));
	    	});
	    	$this.change(file, rand);
	    };
	    this.change = function(file, rand){
	    	if (click) click(options);
	    	var form = $("#uploadImg_form" + rand);
	    	file.change(function(){
	    		form.find("*").remove();
	    		var new_file = file.clone();
	    		$this.change(new_file, rand);
	    		file.before(new_file).appendTo(form);
	    		if (options.formData) {
	    			for(var key in options.formData) form.append('<input type="hidden" name="{0}" value="{1}">'.format(key, options.formData[key]));
	    		}
	    		$(form).submit();
	    	});
	    }
	    this.init = function () {
	        $this.each(function () {
	        	var me = $(this), rand = new Date().getTime();
	        	$("body").append(template_form.format(options.url, rand));

	            me.before(template_box.format(rand, options.title));
	            me.prev().find(".uploadImg-file").width(options.width + 2).height(options.height + 2);
	            me.prev().find(".uploadImg-btn").width(options.width).height(options.height).css('line-height', options.height + 2 + 'px');
	            $this.event(me, rand);
	        });
	    };
